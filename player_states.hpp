...
{
public:
    FirstAttack(Player* player, float speed);
    void update(Player* player, float dt);
    void handleEvents(Player* player, const sf::Event& event);
    void hook(Player* player);
    void startFalling(Player* player);
...
private:
    ...
    float runspeed;
    float mCurrentTime;
    static constexpr float kAttackTime = 0.9;
};

class SecondAttack : public PlayerState
{
public:
    SecondAttack(Player* player, float speed);
    void update(Player* player, float dt);
    void handleEvents(Player* player, const sf::Event& event);
    void hook(Player* player);
    void startFalling(Player* player);
...
private:
    ...
    float runspeed;
    float mCurrentTime;
    static constexpr float kAttackTime = 1;
};

class ThirdAttack : public PlayerState
{
public:
    ThirdAttack(Player* player, float speed);
    void update(Player* player, float dt);
    void handleEvents(Player* player, const sf::Event& event);
    void hook(Player* player);
    void startFalling(Player* player);
